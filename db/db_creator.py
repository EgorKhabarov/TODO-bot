...
status, removal_time, adding_time, recent_changes_time
FROM events LIMIT 1;
"""
        )
    except Error as e:
...
CREATE TABLE events (
    ...
    CONSTRAINT unique_user_id_and_event_id UNIQUE (user_id, event_id)
);
""",
                commit=True,
            )
...
AFTER UPDATE OF date, text, status ON events
FOR EACH ROW
BEGIN
    UPDATE events
